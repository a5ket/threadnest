...
describe('ThreadPresenter', () => {
  ...
  describe('toDetailView', () => {
    it('never exposes which platform admin removed the thread, even to nest moderators', () => {
      ...
      const view = presenter.toDetailView(thread, ctx)

      expect(view.deletedById).toBeUndefined()
      expect(view.deletedByPlatform).toBe(true)
    })

    it('shows deletedById and deletedByPlatform to nest moderators for nest-removed threads', () => {
    ...
    it('hides deletedById and deletedByPlatform from regular (non-moderator) viewers, regardless of who removed it', () => {
      ...
      const platformView = presenter.toDetailView(platformRemoved, ctx)
      const modView = presenter.toDetailView(modRemoved, ctx)
