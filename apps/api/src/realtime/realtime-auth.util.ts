...
// Socket.IO clients normally pass the token via `io(url, { auth: { token } })`; fall back to
// the same header/cookie extraction the HTTP AuthGuard uses for clients that can't set `auth`.
