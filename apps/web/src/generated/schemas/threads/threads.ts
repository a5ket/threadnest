 ...
 */
export const threadDiscoverQueryLimitDefault = 20
export const threadDiscoverQueryLimitMax = 100

export const ThreadDiscoverQueryParams = zod.object({
  limit: zod.number().min(1).max(threadDiscoverQueryLimitMax).default(threadDiscoverQueryLimitDefault),
  cursor: zod.string().optional()
})

export const ThreadDiscoverResponse = zod.object({
  data: zod.object({
    items: zod.array(zod.object({
      id: zod.string().describe('Thread ID'),
      slug: zod.string().describe('Unique thread slug (within its nest)'),
